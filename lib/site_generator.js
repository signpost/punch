...
module.exports = {
	...
	setup: function(config){
		...
		self.templateEngine = ModuleUtils.requireAndSetup(config.plugins.template_engine, config);
		...
		_.each(config.plugins.generator_hooks, function(hook) {
			self.generatorHooks.push(ModuleUtils.requireAndSetup(hook, config));
		});
		...
		var skip_paths = function(from) {
			...
			_.each(self.pathsToSkip, function(skip_pattern) {
				_.each(from, function(path) {
					if (PathUtils.matchPath(path, skip_pattern) !== null) {
	...
	renderPath: function(request_path, callback) {
		...
		var file_extension = PathUtils.getExtension(request_path, []);
	...
	buildBundles: function(callback) {
		...
		return AssetBundler.touchBundles(run_generator_hooks, callback);
	},
